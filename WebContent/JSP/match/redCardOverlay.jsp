	...
	<div class="opciones">
		<table>
			<tr class="subtitulo"><%out.println(request.getAttribute("localname"));%></tr>
				...
				List<RedCard> redCards = (List<RedCard>) request.getAttribute("greenCards");
			 		...
			 		for (int i = 0; i < localplayers.size(); i++){
						...
						if (!redCards.isEmpty()){
			 			for(int j=0; j<redCards.size(); j++){
			 				...
			 				if(redCards.get(j).getPlayer().getIdplayer()==localplayers.get(i).getIdplayer()){
			 		...
			 		for (int i = 0; i < localplayers.size(); i++){
						...
						if (!redCards.isEmpty()){
			 			for(int j=0; j<redCards.size(); j++){
			 				...
			 				if(redCards.get(j).getPlayer().getIdplayer()==guestplayers.get(i).getIdplayer()){
